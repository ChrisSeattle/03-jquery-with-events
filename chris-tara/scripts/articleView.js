...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    ...
    if ($(this).val()) {
      ...
      // only show author they selected, with a fade in. 
    } else {
      ...
      $('article[class="template"]').hide(); // hide the template
      // ISSUE: maybe we could have only shown all articles except class="template"
    }
    $('#category-filter').val('');
  });
};
...
articleView.handleCategoryFilter = function() {
  $('#category-filter').on('change', function() {
    if ($(this).val()) {
      // If the <select> menu was changed to an option that has a value, we first need to hide all the articles, and then show just the ones that match for the category that was selected.
      // Use an "attribute selector" to find those articles, and fade them in for the reader.
      
      $('article').hide(); // hide all articles. 
      $(`article[data-category="${$(this).val()}"]`).fadeIn(1000); 
      // only show category they selected, with a fade in. 
    } else {
    ...
    $('#author-filter').val(''); // reset the author filter to blank. 
  });
...
articleView.setTeasers = function() {
  ...
  $('article').on('click', 'a.read-on', function(e) {
    // contextual this is a refrence to the link they clicked on. 
    e.preventDefault(); 
    $(this).parent().find('*').fadeIn(500); 
    // ISSUE: Do we need to hide the "Read On" button? 

  }); // end event listener for clicking a.read-on somewhere on the page
};
